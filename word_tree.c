...
word_tree_t *filter_dictionary(reference_t *ref, word_tree_t *src, word_tree_t *dst)
{
  ...
  if(src == dst)
  {
    // O(n), but hopefully most times the tree is cut before
    // because of known chars that allows us to skip huge subtrees
    word_tree_filter(src, ref->len, known_chars_filter, words_filter);
  }
  else
  {
    // O(n * len), cause it also has to insert into a new tree, so a bit worse
    word_tree_for_each_ordered(src, ref->len, known_chars_filter, LAMBDA(void, (char *str, int *freq) {
      if(words_filter(str, freq))
        word_tree_push(dst, str);
    }));
  }
...
void populate_dictionary(word_tree_t *tree, word_tree_t *curr_tree, int len, char *stop_command)
{
...
int main()
{
  ...
  populate_dictionary(tree, NULL, len, "+nuova_partita");
  
  for(int quit = 0; !quit; )
  {
    word_tree_t *filtered_tree = new_word_tree();
    word_tree_t *curr_tree = tree;
    ...
    for(;;)
    {
      ...
      if(game_over || line[0] == '+')
      {
        ...
        {
          populate_dictionary(tree, curr_tree == tree ? NULL : curr_tree, len, "+inserisci_fine");
          curr_tree = filter_dictionary(&ref, curr_tree, filtered_tree);
        }
        else if(strcmp(line, "+stampa_filtrate") == 0)
        {
          // print_found_ref(&ref);
          word_tree_for_each_ordered(
              curr_tree, 
              len,
              LAMBDA(int, (int pos, char c, int char_freq) { return VISIT_SUBTREE; }),
              LAMBDA(void, (char *str, int *freq) { printf("%s\n", str); }));
        }
      }
